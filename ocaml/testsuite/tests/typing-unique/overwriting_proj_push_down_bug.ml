...
(let
  ...
  (apply (field_imm 1 (global Toploop!)) "check_heap_alloc_in_overwrite"
    check_heap_alloc_in_overwrite/460))
val check_heap_alloc_in_overwrite : unique_ option_record -> option_record @@
  global many = <fun>
|}]
...
val check_heap_alloc_in_overwrite :
  local_ unique_ option_record -> local_ option_record @@ global many = <fun>
|}]
...
val update : unique_ mutable_record -> string @@ global many = <fun>
|}]
