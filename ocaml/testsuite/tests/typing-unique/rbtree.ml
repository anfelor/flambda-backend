...
module Make_Unique_Okasaki :
  functor (Ord : Map.OrderedType) ->
    sig
      ...
      val set_black : unique_ (Ord.t, '_weak1) tree -> unique_ ('a, 'b) tree
        @@ global many
      val set_red : unique_ (Ord.t, '_weak1) tree -> unique_ ('a, 'b) tree @@
        global many
      val balance_left :
        unique_ (Ord.t, '_weak1) tree -> unique_ (Ord.t, '_weak1) tree @@
        global many
      val balance_right :
        unique_ (Ord.t, '_weak1) tree -> unique_ (Ord.t, '_weak1) tree @@
        global many
      val ins :
        Ord.t ->
        '_weak1 -> unique_ (Ord.t, 'a) tree -> unique_ (Ord.t, '_weak1) tree
        @@ global many
      val insert :
        Ord.t -> '_weak1 -> unique_ (Ord.t, 'a) tree -> ('b, 'c) tree @@
        global many
    end
type tree_tag = private Tree
type zipper_tag = private Zipper
...
module Make_Unique_Okasaki :
  functor (Ord : Map.OrderedType) ->
    sig
      ...
      val insert :
        Ord.t ->
        '_weak1 -> unique_ (Ord.t, '_weak1) tree -> (Ord.t, '_weak1) tree @@
        global many
    end
type tree_tag = private Tree
type zipper_tag = private Zipper
