...
val unique_work :
  ('a : value_or_null) ('b : value_or_null) ('c : value_or_null).
    insert:(int -> bool -> unique_ 'a -> unique_ 'a) ->
    ...
    sig
      ...
      val fold :
        'a 'b ('c : value_or_null).
          ('a -> 'b -> 'c -> 'c) -> 'c -> ('a, 'b) tree -> 'c
        @@ global many
      val set_color : color -> unique_ ('a, 'b) tree -> unique_ ('c, 'd) tree
        @@ global many portable
      val set_black : unique_ ('a, 'b) tree -> unique_ ('c, 'd) tree @@
        global many portable
      val set_red : unique_ ('a, 'b) tree -> unique_ ('c, 'd) tree @@ global
        many portable
      val balance_left : unique_ ('a, 'b) tree -> unique_ ('a, 'b) tree @@
        global many portable
      val balance_right : unique_ ('a, 'b) tree -> unique_ ('a, 'b) tree @@
        global many portable
      val ins :
        Ord.t -> 'a -> unique_ (Ord.t, 'b) tree -> unique_ (Ord.t, 'a) tree
        @@ global many
      val insert : Ord.t -> 'a -> unique_ 'a t -> unique_ 'a t @@ global many
    end
type tree_tag = private Tree
type zipper_tag = private Zipper
type ('left, 'right, 'kind) tag =
  ...
  functor (Ord : Map.OrderedType) ->
    sig
      ...
      val fold :
        ...
        global many
      val balance_left :
        unique_ ('a, 'b, tree_tag) tagged_tree -> unique_
        ('a, 'b, tree_tag) tagged_tree @@ global many portable
      val balance_right :
        unique_ ('a, 'b, tree_tag) tagged_tree -> unique_
        ('a, 'b, tree_tag) tagged_tree @@ global many portable
      val ins : Ord.t -> 'a -> unique_ 'a t -> unique_ 'a t @@ global many
      val set_black :
        unique_ ('a, 'b, tree_tag) tagged_tree -> unique_
        ('a, 'b, tree_tag) tagged_tree @@ global many portable
      val insert : Ord.t -> 'a -> unique_ 'a t -> unique_ 'a t @@ global many
  ...
  functor (Ord : Map.OrderedType) ->
    sig
      ...
      val fold :
        ('a -> 'b -> 'c -> 'c) -> 'c -> ('a, 'b, 'd) tagged_tree -> 'c @@
        global many
      val move_up : unique_ 'a z -> unique_ 'a t -> unique_ 'a t @@ global
        many portable
      val fixup : unique_ 'a z -> unique_ 'a t -> unique_ 'a t @@ global many
        portable
      val ins : Ord.t -> 'a -> unique_ 'a z -> unique_ 'a t -> unique_ 'a t
        @@ global many
      val set_black :
        unique_ ('a, 'b, tree_tag) tagged_tree -> unique_
        ('a, 'b, tree_tag) tagged_tree @@ global many portable
    ...
    sig
      ...
      val fold :
        'a 'b ('c : value_or_null).
          ('a -> 'b -> 'c -> 'c) -> 'c -> ('a, 'b) tree -> 'c
        @@ global many
      val set_color : color -> unique_ ('a, 'b) tree -> unique_ ('a, 'b) tree
        @@ global many portable
      val set_black : unique_ ('a, 'b) tree -> unique_ ('a, 'b) tree @@
        global many portable
      val set_red : unique_ ('a, 'b) tree -> unique_ ('a, 'b) tree @@ global
        many portable
      val balance_left : unique_ ('a, 'b) tree -> unique_ ('a, 'b) tree @@
        global many portable
      val balance_right : unique_ ('a, 'b) tree -> unique_ ('a, 'b) tree @@
        global many portable
      val ins :
        Ord.t -> 'a -> unique_ (Ord.t, 'a) tree -> unique_ (Ord.t, 'a) tree
        @@ global many
      val insert : Ord.t -> 'a -> unique_ 'a t -> unique_ 'a t @@ global many
    end
type tree_tag = private Tree
