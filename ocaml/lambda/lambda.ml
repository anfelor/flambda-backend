...
let primitive_may_allocate : primitive -> locality_mode option = function
  ...
  | Preuseblock { mode } -> Some (fst mode)
  | Preusefloatblock { mode } -> Some (fst mode)
  | Preuseufloatblock { mode } -> Some (fst mode)
  | Preusemixedblock { mode } -> Some (fst mode)
  ...
  | Parrayrefs ((Pgenarray_ref m | Pfloatarray_ref m), _) -> Some (fst m)
  ...
  | Punboxed_nativeint_array_load_128 { mode = m; _ } -> Some (fst m)
  ...
  | Pbigstring_load_128 { mode = m; boxed = true; _ } -> Some (fst m)
