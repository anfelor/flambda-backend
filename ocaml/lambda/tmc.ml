...
module Constr : sig
  ...
  val makeblock : (int * Lambda.mutable_flag * block_shape * is_reusing * Lambda.alloc_mode) -> primitive
  ...
  let apply constr t =
    ...
    let block = makeblock (constr.tag, constr.flag, constr.shape, constr.is_reusing, alloc_heap_aliased) in
...
let rec choice ctx t =
  ...
  and choice_prim ctx ~tail prim primargs loc =
    ...
    | Preuseblock { tag; mut; shape; resets; mode } ->
      choice_makeblock ctx ~tail (tag, mut, shape, Constr.Reuse_alloc { resets }, mode) primargs loc
