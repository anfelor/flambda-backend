...
module Maybe_shared : sig
  ...
  let add_barrier t uniq =
    List.iter (fun (barrier, _, _) -> barrier := Uniqueness.meet [!barrier; uniq]) t
end
...
module Paths : sig
  ...
  val mark_implicit_borrow_memory_address :
    unique_barrier ref -> Maybe_shared.access -> Occurrence.t -> t -> UF.t
...
module Value : sig
  ...
  (** Mark the memory_address of the value as implicitly borrowed
      (borrow_or_shared). *)
  val mark_implicit_borrow_memory_address :
    unique_barrier ref -> Maybe_shared.access -> t -> UF.t
...
and pattern_match_single pat paths : Ienv.Extension.t * UF.t =
  ...
  | Tpat_constant _ ->
    ( Ienv.Extension.empty,
      Paths.mark_implicit_borrow_memory_address pat.pat_unique_barrier Read occ paths )
  | Tpat_construct (lbl, cd, pats, _) ->
    let uf_read =
      Paths.mark_implicit_borrow_memory_address pat.pat_unique_barrier Read occ paths in
...
let rec check_uniqueness_exp (ienv : Ienv.t) exp : UF.t =
  ...
  | Texp_record { fields; extended_expression; unique_barrier } ->
    let value, uf_ext =
      ...
      | Some exp ->
        ...
        let uf_read = Value.mark_implicit_borrow_memory_address unique_barrier Read value in
  ...
  | Texp_setfield (rcd, _, _, _, arg, unique_barrier) ->
    ...
    let uf_write = Value.mark_implicit_borrow_memory_address unique_barrier Write value in
